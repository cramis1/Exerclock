...
export class ViewExercise extends View {
  ...
  elBoxStats = $("#boxStats");
  lblSpeedrev = $("#lblSpeedrev");
  lblSpeedrevUnits = $("#lblSpeedrevUnits");
  lblSpeed = $("#lblSpeed");
  ...
  handleFinish = () => {
    ...
    let popupSettings = {
      ...
      message: `Are you sure you want to finish this ${
        config.exerciseName[exerciseSelect._index]
      } session?`,
      btnLeftLabel: "Cancel",
  ...
  handleLocationSuccess = () => {
    ...
    //let exerciseSelect = fs.readFileSync("/private/data/Exercise.txt", "json");
   // exercise.start(config.exerciseName[exerciseSelect._index], { gps: true });
    this.lblStatus.text = "";
    this.gps.callback = undefined;
    display.poke();
  ...
  onMount() {
   ...
   // utils.hide(this.btnToggle);
    ...
    let exerciseSelect2 = fs.readFileSync("/private/data/Exercise.txt", "cbor");
    console.log("exerciseSelect2:" + exerciseSelect2._index)
    if ((exerciseSelect2._index === 0) || (exerciseSelect2._index === 4) || (exerciseSelect2._index === 15) || (exerciseSelect2._index === 16) ) {
    console.log("get GPS from subview")
    let exerciseSelect = fs.readFileSync("/private/data/Exercise.txt", "cbor");
     exercise.start(config.exerciseName[exerciseSelect._index], { gps: true });  
    this.gps = new GPS("#subview-gps2", this.handleLocationSuccess);
    this.insert(this.gps);
    } else {
     console.log("do not get GPS from subview")
     utils.show(this.btnToggle);
     let exerciseSelect = fs.readFileSync("/private/data/Exercise.txt", "cbor");
     exercise.start(config.exerciseName[exerciseSelect._index], { gps: false });
     this.lblStatus.text = "";
    // this.gps.callback = undefined;
     display.poke();
    }
  ...
  onRender() {
    if (exercise && exercise.stats) {
      ...
      if (speed.value < 0.1) { this.lblSpeedrev.text = '0' } else { 
      this.lblSpeedrev.text = Math.round((60/(speed.value))*100)/100;}
      this.lblSpeedrevUnits.text = `speed minutes/${distance.units}`;
      ...
      let exerciseSelect3 = fs.readFileSync("/private/data/Exercise.txt", "cbor");
      if ((exerciseSelect3._index === 0) || (exerciseSelect3._index === 4) || (exerciseSelect3._index === 15) || (exerciseSelect3._index === 16) ) {

      kmInterval = setInterval(function() {
        let z = distance.value % 1;
        if (z === 0) {
